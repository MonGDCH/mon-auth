...
class Token
{
    ...
    public function parse(string $jwt, string $key, string $alg = 'HS256'): array
    {
        $ticket = explode('.', $jwt);
        if (count($ticket) != 3) {
            throw new JwtException('无效的格式', JwtException::JWT_FORMAT_ERROR);
        }
        list($head, $body, $crypt) = $ticket;
        $header = json_decode($this->urlsafeB64Decode($head), true);
        if (!$header) {
            throw new JwtException('无效的header编码', JwtException::JWT_HEADER_ERROR);
        }
        $payload = json_decode($this->urlsafeB64Decode($body), true);
        if (!$payload) {
            throw new JwtException('无效的payload编码', JwtException::JWT_PAYLOAD_ERROR);
        }
        $sign = $this->urlsafeB64Decode($crypt);
        if (!$sign) {
            throw new JwtException('无效的sign签名编码', JwtException::JWT_SIGN_ERROR);
        }
        // 验证加密方式
        if (!isset($this->algs[$header['alg']])) {
            throw new JwtException('未定义加密方式', JwtException::ALG_NOT_FOUND);
        }
        if ($header['alg'] != $alg) {
            throw new JwtException('加密算法不支持', JwtException::ALG_NOT_SUPPORT);
        }
        // 验证签名有效性
        if (!$this->examine("{$head}.{$body}", $sign, $key, $alg)) {
            throw new JwtException('Token签名错误', JwtException::TOKEN_VERIFY_ERROR);
        }
    ...
    public function verify(array $payload): bool
    {
        ...
        if (isset($payload['nbf']) && $payload['nbf'] > $now) {
            throw new JwtException('Token签名未生效', JwtException::TOKEN_NBF_ERROR);
        }
    ...
    public function sign(string $info, string $key, string $alg = 'HS256'): string
    {
        ...
        switch ($type) {
            ...
            case 'openssl':
                ...
                if (!$success) {
                    // 不存在openssl加密扩展
                    throw new JwtException('openssl无法签名数据', JwtException::OPENSSL_ERROR);
                } else {
                    return $signature;
                }
            default:
                throw new JwtException("加密算法未支持", JwtException::ALG_NOT_SUPPORT);
        }
    }
    ...
    protected function examine(string $info, string $sign, string $key, string $alg = 'HS256'): bool
    {
        ...
        switch ($type) {
            case 'openssl':
                ...
                throw new JwtException('openssl error: ' . openssl_error_string(), JwtException::OPENSSL_ERROR);
            case 'hash_hmac':
